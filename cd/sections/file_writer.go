...
func (sect *Section) WriteSubSection(opt *option.Option) {
	...
	subFile, err := os.OpenFile(subFileName, os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0o640)
	if err != nil {
		panic(err)
	}
	defer subFile.Close()
...
func (sect *Section) WriteHash(opt *option.Option) {
	...
	hashFile, err := os.OpenFile(hashFileName, os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0o640)
	if err != nil {
		panic(err)
	}
